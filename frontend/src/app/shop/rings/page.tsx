...
export default function RingsPage() {
  ...
  useEffect(() => {
    fetch('http://127.0.0.1:8000/api/products')
      .then((res) => res.json())
      .then((data) => {
        const filtered = data.filter((p: any) => p.category.toLowerCase() === 'rings');
  ...
  if (loading) return <div style={{ textAlign: 'center', padding: '150px', color: '#0f3c2b' }}>LOADING RINGS VAULT...</div>;
    ...
    <div style={{ width: '100%', maxWidth: '1600px', margin: '0 auto', padding: '60px 40px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', borderBottom: '1px solid #111', paddingBottom: '20px', marginBottom: '40px' }}>
        <div>
          ...
          <h1 style={{ color: '#0f3c2b', fontSize: '3rem', fontFamily: 'serif', margin: '10px 0 0 0', fontWeight: 'normal' }}>Statement Rings</h1>
        </div>
